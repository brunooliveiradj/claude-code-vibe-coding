...
import { LogIn, ShieldAlert, Mail, Lock, Eye, EyeOff, ArrowLeft, CheckCircle } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 p-4">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl max-w-md w-full text-center space-y-8 border border-zinc-100">
              ...
              <form onSubmit={handleEmailLogin} className="space-y-4 text-left">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between ml-1">
                    ...
                    <button
                      type="button"
                      onClick={() => { setView('recovery'); setRecoveryEmail(email); setError(null); }}
                      className="text-[10px] font-bold text-adsplay hover:underline"
                    >
                      Esqueceu a senha?
                    </button>
                  </div>
          ...
          {view === 'recovery' && (
            <>
              <div className="w-16 h-16 bg-adsplay/10 text-adsplay rounded-2xl flex items-center justify-center mx-auto">
                <Mail size={32} />
              </div>
              <div className="space-y-2">
                <h1 className="text-2xl font-black tracking-tighter">Recuperar acesso</h1>
                <p className="text-zinc-500 font-medium text-sm">Digite seu e-mail e enviaremos um link para redefinir sua senha.</p>
              </div>
              <form onSubmit={handleRecovery} className="space-y-4 text-left">
                ...
                <button
                  ...
                  className="w-full py-4 bg-adsplay text-white rounded-2xl font-bold hover:bg-adsplay-dark transition-all shadow-lg shadow-adsplay/20 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="w-full py-3 flex items-center justify-center gap-2 text-zinc-500 hover:text-zinc-700 font-medium text-sm transition-colors"
                >
          ...
          {view === 'recovery-sent' && (
            <>
              ...
              <div className="space-y-2">
                <h1 className="text-2xl font-black tracking-tighter">E-mail enviado!</h1>
                <p className="text-zinc-500 font-medium text-sm">
                  Verifique sua caixa de entrada em <span className="font-bold text-zinc-700">{recoveryEmail}</span> e clique no link para criar uma nova senha.
                </p>
              </div>
              <button
                onClick={() => { setView('login'); setError(null); }}
                className="w-full py-4 flex items-center justify-center gap-2 bg-zinc-900 text-white rounded-2xl font-bold hover:bg-zinc-800 transition-all"
              >
      ...
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 p-4">
        <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full text-center space-y-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-zinc-500">Seu e-mail ({user.email}) não está autorizado. Entre em contato com um administrador.</p>
          </div>
          <button
            onClick={logout}
